...
class Codepoint {
    ...
    protected $image;
    protected $fonts;
    protected static $cp_cache = [];

    public static $defaultImage = 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABAAAAAQCAAAAAA6mKC9AAAAQUlEQVQY022PSQ4AIAgD5/+fxhhbEJEL6bAV4gmc6QBMSC1C9otQ9UOwRntoeqdommsEj8iED+ssae1vbFqfz1Usi/eYaGRQ6NgAAAAASUVORK5CYII=';
     ...
     */
    protected function __construct($id, $db, $info=[]) {
        $this->id = $id;
        $this->db = $db;
        foreach ($info as $k => $v) {
            $this->$k = $v;
        ...
        if ($this->confusables === NULL) {
            ...
            foreach ($conf as $v) {
                if (! array_key_exists($v['type'], $base)) {
                    $base[$v['type']] = []; // any of SL, SA, ML, MA
                }
    ...
    public function getFonts() {
        if ($this->fonts === NULL) {
            ...
            $query->execute([$this->id]);
            $this->fonts = $query->fetchAll(\PDO::FETCH_ASSOC);
            $query->closeCursor();
        }
        return $this->fonts;
    }
    ...
    public function getProp($prop, $default=NULL) {
        ...
        if (array_key_exists($prop, $props)) {
            if (in_array($prop, ['bmg','suc','slc','stc','scf'])) {
                if (intval($v) === $this->id) {
                    return $this;
                } else {
                    return self::getCP(intval($props[$prop]), $this->db);
                }
            } elseif (in_array($prop, ['uc','lc','tc','cf','dm',
                                       'FC_NFKC','NFKC_CF'])) {
                $r = [];
    ...
    public function getPronunciation() {
        ...
        if ($toPinyin) {
            $pr = preg_replace_callback('/([aeiouü])([^aeiouü12345]*)([12345])/',
                function($matches) {
                    $map = [
                        'a' => [1 => '0101', 2 => '00E1', 3 => '01CE', 4 => '00E0'],
                        'e' => [1 => '0113', 2 => '00E9', 3 => '011B', 4 => '00E8'],
                        'i' => [1 => '012B', 2 => '00ED', 3 => '01D0', 4 => '00EC'],
                        'o' => [1 => '014D', 2 => '00F3', 3 => '01D2', 4 => '00F2'],
                        'u' => [1 => '016B', 2 => '00FA', 3 => '01D4', 4 => '00F9'],
                        'ü' => [1 => '01D6', 2 => '01D8', 3 => '01DA', 4 => '01DC'],
                    ];
     ...
     */
    public static function getCP($cp, $db, $info=[]) {
        if (is_string($cp)) {
            $cp = intval($cp);
        }
